...
**任务跑完，邮件到你手机；新通知一到，上一封自动消失。**

给 [DeepSeek Harness](https://github.com/deepseek-ai/deepseek-harness)（dsh）用的任务完成通知插件。零依赖、零构建、不监听端口。
...
| 出错 / 中断 | ✅ 必发，高优先级 |
| 目标（goal）完成 | ✅ 必发 |
| 单轮耗时超过 `longTaskMs`（默认 5 分钟） | ✅ 发 |
| 普通问答、闲聊、几秒钟的小活儿 | ❌ 不发 |

想更安静就设 `notifyOn: "errors"`（只报错）；想每轮都收就设 `"all"`。

> **提示**：`goals` 档依赖 dsh 的 goal 机制。接到长任务时让 agent 先 `create_goal`，完成时你才会收到通知。
...
| `longTaskMs` | `300000` | 超过这个毫秒数的轮次算"正经干活"，也会通知 |
...
也可以用环境变量覆盖：`DSH_NOTIFY_CHANNEL`、`DSH_NOTIFY_ON`、`DSH_NOTIFY_SMTP_HOST`、`DSH_NOTIFY_SMTP_USER`、`DSH_NOTIFY_SMTP_PASS`、`DSH_NOTIFY_MAIL_TO` 等（完整列表见源码 `resolveConfig`）。
...
**安全吗？**
授权码只存在你本机的配置文件里，不会上传到任何地方。它只能用来收发你自己的邮件，随时可以在邮箱设置里作废重发。插件本身不监听端口、不向任何第三方服务发送数据（除非你开了 ntfy）。

## 开发

```bash
node test-logic.mjs     # dryrun 验证"什么情况才推送"，不会真发邮件
